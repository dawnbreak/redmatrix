...
class RedDirectory extends DAV\Node implements DAV\ICollection, DAV\IQuota /*, DAVACL\IACL */ {
	...
	public function createFile($name, $data = null) {
		...
		/** @todo move this to own quota plugin */
	 ...
	 *
	 * @fixme Should guests relly see the used/free values from filesystem of
	 * the complete store directory?
	 *
	...
	/** @copydoc ACLTrait::getACL() */
	function getACL() {
		// owner has all privileges
		$acl = $this->getOwnerACL();

		//
		if (! $this->auth->owner_id > 0) {
			return $acl;
		}

		if (perm_is_allowed($this->auth->owner_id, $this->auth->observer, 'read_storage')) {
			$acl[] = [
				'privilege' => '{DAV:}read',
				'principal' => '{DAV:}principals/channel/' . $this->auth->observer,
				'protected' => true
			];
		}
		if (perm_is_allowed($this->auth->owner_id, $this->auth->observer, 'write_storage')) {
